document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('data/prayerTimes.json')
    ...
    .then(prayerData => {
      ...
      if (todaysData) {
        ...
        document.getElementById('isha-time').textContent    = todaysData.Isha || todaysData.Esha; // support either key

        // Show today's date
        document.getElementById('today-date').textContent = `Today's Date: ${todayDate}`;
      } else {
        // No timetable found for today's date
  ...
  menuToggle.addEventListener('click', () => {
    navLinks.classList.toggle('show'); // toggles the 'show' class for the nav links
  });
});
